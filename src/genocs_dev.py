...
def config_personas():
    ...
    engineer = AssistantAgent(
        name="Engineer",
        system_message='''Engineer. You follow an approved plan. You write CSharp code along with Genocs Library code to solve tasks. Wrap the 
...
def start_chat_genocs_dev(message, is_test=False):
    if not is_test:
...
if __name__ == "__main__":
    test_message = (
        "I would like to build a simple e-commerce WebAPI that allows to setup a "
        "product.")
    start_chat_genocs_dev(test_message, is_test=True)
